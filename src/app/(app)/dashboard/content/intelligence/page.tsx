...
type Metric = { content_id: string; captured_at: string; reach: number | string; engagements: number | string; clicks: number | string; leads: number | string };
...
export default async function ContentIntelligencePage() {
  ...
  const [metricResult, learningResult] = await Promise.all([
    supabase.from("content_metric_snapshots").select("content_id,captured_at,reach,engagements,clicks,leads").eq("workspace_id", workspace.id).gte("captured_at", since).order("captured_at", { ascending: false }),
  ...
  if (metricResult.error || learningResult.error) throw new Error("Content Intelligence could not be loaded completely.");

  const rawMetrics = (metricResult.data ?? []) as Metric[];
  ...
  const ids = metrics.map((item) => item.content_id);
  let drafts: Draft[] = [];
  if (ids.length) {
    ...
    if (draftResult.error) throw new Error("Measured content metadata could not be loaded.");
  ...
  const byChannel = new Map<string, { reach: number; engagements: number; clicks: number; leads: number; items: number }>();
  for (const metric of metrics) {
    const channel = draftById.get(metric.content_id)?.channel || "unknown";
    const current = byChannel.get(channel) ?? { reach: 0, engagements: 0, clicks: 0, leads: 0, items: 0 };
    current.reach += num(metric.reach);
    current.engagements += num(metric.engagements);
    current.clicks += num(metric.clicks);
    current.leads += num(metric.leads);
    current.items += 1;
