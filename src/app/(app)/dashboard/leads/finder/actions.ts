...
function apiKey() {
  ...
  if (!key) {
    finderRedirect(
      "error",
      "Google Places is not connected yet. Add GOOGLE_PLACES_API_KEY in Vercel.",
    );
  }
  return key;
}
...
export async function searchPlaces(formData: FormData) {
  ...
  if (!parsed.success) {
    finderRedirect("error", "Check the search brief and try again.");
  }
  ...
  if (searchError || !search) {
    finderRedirect("error", "Orbit could not create the discovery search.");
  }

  let completionMessage = "Lead Finder completed the search.";
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Google Places ${response.status}: ${body}`);
    }
    ...
    if (placeIds.length) {
      ...
      if (resultsQuery.error || leadsQuery.error || memoryQuery.error) {
        throw new Error("Orbit could not check duplicate Place IDs.");
      }
    ...
    const newPlaces = places.filter(
      (place) => !known.has(place.id as string),
    );
    ...
    completionMessage = `${newPlaces.length} new opportunities found; ${places.length - newPlaces.length} known or previously rejected businesses skipped.`;
  } catch (error) {
    await supabase
      .from("lead_finder_searches")
      .update({
    ...
    finderRedirect(
      "error",
      "Lead Finder could not complete the search. Check the Google Places key, billing and API access.",
    );
  }
...
export async function analyzeFinderResult(formData: FormData) {
  ...
  const { data, error } = await supabase
    .from("lead_finder_results")
    .select(
      "id, provider_place_id, business_name, formatted_address, primary_type, google_maps_url, niche, target_problem, status",
    )
    .eq("id", id.data)
    .eq("workspace_id", workspace.id)
    .single();
  if (error || !data) {
    finderRedirect("error", "Discovery result not found.");
  }
  const result = data as FinderResult;
  if (["approved", "rejected", "duplicate"].includes(result.status)) {
    finderRedirect("error", "This opportunity has already been decided.");
  }

  let place: GooglePlace;
  try {
    const response = await fetch(
      `https://places.googleapis.com/v1/places/${encodeURIComponent(result.provider_place_id)}`,
      {
        headers: {
          "X-Goog-Api-Key": key,
          "X-Goog-FieldMask":
            "id,displayName,formattedAddress,nationalPhoneNumber,websiteUri,rating,userRatingCount,businessStatus,googleMapsUri,primaryType",
        },
        cache: "no-store",
      },
    );
    if (!response.ok) {
      throw new Error(`Google Places detail request failed: ${response.status}`);
    }
    place = (await response.json()) as GooglePlace;
  } catch {
    finderRedirect(
      "error",
      "Google Places could not enrich this business. Check the provider connection and try again.",
    );
  }
  ...
  const { error: updateError } = await supabase
    ...
    .update({
      ...
      formatted_address:
        optional(place.formattedAddress) ?? result.formatted_address,
  ...
  if (updateError) {
    finderRedirect(
      "error",
      "Orbit could not save the opportunity analysis.",
    );
  }
...
export async function approveFinderResult(formData: FormData) {
  ...
  if (data.status !== "analyzed") {
    finderRedirect("error", "Analyze this opportunity before approving it.");
  }
  ...
  if (duplicate) {
    await Promise.all([
      ...
      "This business already exists in the Lead Engine and was marked duplicate.",
    );
  }
  ...
  const notes = [
    ...
    data.rating !== null
      ? `Google Maps rating signal: ${data.rating} from ${data.review_count ?? 0} ratings.`
      : null,
  ...
  if (insertError || !lead) {
    finderRedirect(
      "error",
      "Orbit could not approve this opportunity into the Lead Engine.",
    );
  }
  ...
  try {
    ...
    finderRedirect(
      "error",
      "Orbit could not preserve the approval decision, so the new lead was rolled back.",
    );
  }
  ...
  if (resultUpdateError) {
    finderRedirect(
      "error",
      "The lead was approved, but Orbit could not update the review card. Refresh the page.",
    );
  }
...
export async function rejectFinderResult(formData: FormData) {
  ...
  if (data.status === "approved") {
    finderRedirect("error", "An approved opportunity cannot be rejected here.");
  }
  ...
  try {
    ...
    finderRedirect(
      "error",
      "Orbit could not preserve this rejection decision.",
    );
  }
  ...
  const { error } = await supabase
    ...
    .eq("workspace_id", workspace.id);
  if (error) {
    finderRedirect("error", "Orbit could not reject this opportunity.");
  }
  done("Opportunity rejected. Orbit will remember the Place ID.");
}
