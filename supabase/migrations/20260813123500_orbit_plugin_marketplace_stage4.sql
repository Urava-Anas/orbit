...
-- Bootstrap the existing Orbit product owner by stable auth user id, not by a runtime email comparison.
...
-- Add update-review state without widening any existing privileges.
...
alter table public.plugin_installations
  add constraint plugin_installations_status_check
...
create policy plugin_publishers_select_member on public.plugin_publishers
  ...
  using ((select private.is_workspace_member(plugin_publishers.workspace_id)));
...
create policy plugin_submissions_select_member on public.plugin_submissions
  for select to authenticated
  using ((select private.is_workspace_member(plugin_submissions.workspace_id)));
...
create policy plugin_versions_select_authenticated on public.plugin_versions
  ...
  using (exists (
    ...
    where catalog.id = plugin_versions.plugin_id
      and catalog.status in ('published','deprecated')
  ));
...
set search_path = ''
as $$
declare publisher_workspace uuid;
begin
  select workspace_id into publisher_workspace
  from public.plugin_publishers where id = new.publisher_id;
  ...
  if new.manifest ->> 'id' is distinct from new.proposed_slug then
    raise exception 'Manifest id must match proposed slug';
  end if;
  if new.manifest ->> 'version' is distinct from new.proposed_version then
    raise exception 'Manifest version must match proposed version';
  end if;
  if new.manifest ->> 'schema_version' is distinct from '1' then
    raise exception 'Unsupported plugin manifest schema';
  end if;
  if new.manifest_hash <> encode(extensions.digest(new.manifest::text, 'sha256'), 'hex') then
    raise exception 'Manifest hash does not match manifest';
  end if;
  if tg_op = 'UPDATE' and old.review_status in ('approved','rejected') then
    raise exception 'Reviewed plugin submissions are immutable';
  end if;
  if tg_op = 'UPDATE' and old.review_status = 'submitted' and (
    new.manifest is distinct from old.manifest or
    new.manifest_hash is distinct from old.manifest_hash or
    new.proposed_slug is distinct from old.proposed_slug or
    new.proposed_version is distinct from old.proposed_version or
    new.publisher_id is distinct from old.publisher_id
  ) then
    raise exception 'Submitted plugin manifests cannot be changed';
  end if;
  if new.review_status = 'submitted' and old.review_status = 'draft' then
    new.submitted_at := coalesce(new.submitted_at, now());
  elsif tg_op = 'UPDATE' and new.review_status <> old.review_status and new.review_status not in ('draft','submitted') then
    -- Only the service-role promotion/rejection functions may create terminal review states.
    if coalesce(current_setting('request.jwt.claim.role', true), '') <> 'service_role' then
      raise exception 'Only Orbit review service can set terminal plugin review states';
    end if;
  end if;
  return new;
end;
...
as $$
begin
  raise exception 'Approved plugin versions are immutable';
end;
$$;
...
begin
  if not exists (
    select 1 from public.orbit_platform_admins admin
    where admin.user_id = target_reviewer_id
  ) then
  ...
  select * into publisher_row
  from public.plugin_publishers
  where id = submission_row.publisher_id
  for update;
  if publisher_row.id is null or publisher_row.status <> 'active' then
    raise exception 'Plugin publisher is not active';
  end if;
  ...
  if submission_row.manifest ? 'mcp' then
    ...
    if mcp_url_value is null or mcp_url_value !~ '^https://[^/@:]+(?::443)?(?:/|$)' then
      raise exception 'MCP endpoint must use public HTTPS without URL credentials';
    end if;
  end if;
  ...
  where slug = submission_row.proposed_slug
  for update;
  ...
  if plugin_row.id is null then
    insert into public.plugin_catalog(
      ...
      publisher_row.display_name,
      publisher_row.website,
      submission_row.proposed_version,
      'published',
      true,
      false,
      submission_row.manifest,
      publisher_row.id
    ) returning id into plugin_id_value;
  else
    if plugin_row.publisher_id is distinct from publisher_row.id then
      raise exception 'Plugin slug belongs to another publisher';
    end if;
    if plugin_row.current_version = submission_row.proposed_version then
      raise exception 'Plugin version is already current';
    end if;
    select coalesce(array_agg(permission order by permission), '{}'::text[])
    ...
    escalated := exists (
      select 1 from unnest(permissions_value) permission
      where not (permission = any(previous_permissions))
    ...
    update public.plugin_catalog set
      ...
      developer_name = publisher_row.display_name,
      developer_url = publisher_row.website,
      current_version = submission_row.proposed_version,
      status = 'published',
      verified = true,
      manifest = submission_row.manifest,
  ...
  insert into public.plugin_versions(
    plugin_id,publisher_id,version,manifest,manifest_hash,permissions,mcp_url,approved_by
  ) values (
    plugin_id_value,publisher_row.id,submission_row.proposed_version,submission_row.manifest,
  ...
  if plugin_row.id is not null then
    if escalated then
      ...
      update public.plugin_installations
      set version = submission_row.proposed_version, updated_at = now()
      where plugin_id = plugin_id_value and status in ('installed','pending_connections');
    end if;
  end if;
...
begin
  ...
  update public.plugin_submissions set
    review_status = 'rejected', reviewed_by = target_reviewer_id,
    review_notes = target_review_notes, reviewed_at = now(), updated_at = now()
...
-- First-party publisher/version backfill so runtime consumes immutable versions too.
...
update public.plugin_catalog catalog
...
select
  catalog.id,catalog.publisher_id,catalog.current_version,catalog.manifest,
  ...
  '1603309b-da17-403f-8c2b-a8639789567d'::uuid,
  now()
