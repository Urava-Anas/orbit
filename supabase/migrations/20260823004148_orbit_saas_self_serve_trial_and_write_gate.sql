create or replace function private.orbit_workspace_can_write(target_workspace_id uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select coalesce((
    select case
      when s.status in ('active', 'comped') then true
      when s.status = 'trialing' then s.trial_ends_at is not null and s.trial_ends_at > now()
      else false
    end
...
begin
  ...
  loop
    ...
    execute format(
      'create policy orbit_subscription_insert_gate on public.%I as restrictive for insert to authenticated with check ((select private.orbit_workspace_can_write(workspace_id)))',
      target.table_name
    );

    execute format('drop policy if exists orbit_subscription_update_gate on public.%I', target.table_name);
    execute format(
      'create policy orbit_subscription_update_gate on public.%I as restrictive for update to authenticated using ((select private.orbit_workspace_can_write(workspace_id))) with check ((select private.orbit_workspace_can_write(workspace_id)))',
      target.table_name
    );

    execute format('drop policy if exists orbit_subscription_delete_gate on public.%I', target.table_name);
    execute format(
      'create policy orbit_subscription_delete_gate on public.%I as restrictive for delete to authenticated using ((select private.orbit_workspace_can_write(workspace_id)))',
      target.table_name
    );
  end loop;
end;
