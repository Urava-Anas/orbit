-- Provider secrets are written only through trusted Orbit server code.
-- User authorization is enforced by the server action before this RPC is called.
...
begin
  ...
  if p_value is null or length(trim(p_value)) = 0 or length(p_value) > 10000 then
    raise exception 'invalid provider secret value';
  end if;
  ...
  if secret_id is null then
    perform vault.create_secret(
      p_value,
      secret_name,
      'Orbit Stage 4 encrypted provider setting for workspace ' || p_workspace_id::text
    );
  else
